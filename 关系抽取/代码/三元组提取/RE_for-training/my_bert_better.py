...
def evaluate(config, model, tokenizer, prefix=""):
    ...
    #print(preds.shape,out_label_ids)
...
def load_and_cache_examples(config, task, tokenizer, evaluate=False):
    ...
    cached_features_file = os.path.join(config.data_dir, 'cached_{}_{}_{}_{}'.format(
        ...
        list(filter(None, 'bert-base-chinese'.split('/'))).pop(),
        str(config.max_seq_len),
        str(task)))
    if os.path.exists(cached_features_file):  #已处理好的文本特征文件
...
def main():
    ...
    bertconfig = BertConfig.from_pretrained(
         "/home/chenyanguang/RE/bert-pipeline/RoBERTa_pretrain", num_labels=num_labels, finetuning_task=config.task_name,output_hidden_states=True)
    print(config)
    print(bertconfig)
    bertconfig.l2_reg_lambda = config.l2_reg_lambda
    tokenizer = BertTokenizer.from_pretrained(
         "/home/chenyanguang/RE/bert-pipeline/RoBERTa_pretrain", do_lower_case=True, additional_special_tokens=additional_special_tokens)
    model = BertForSequenceClassification.from_pretrained(
         "/home/chenyanguang/RE/bert-pipeline/RoBERTa_pretrain", config=bertconfig)
