...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, e1_mask=None, e2_mask=None, labels=None,
        ...
        sequence_output = outputs[0]#outputs[-1]
        
        
        #encoded_layers3 = torch.selu(torch.matmul( sequence_output[-3], self.W_w1) + torch.matmul( sequence_output[-2], self.W_w2) + torch.matmul( sequence_output[-1], self.W_w3))
        #print(3,encoded_layers3.shape)
        #encoded_layers= self.dropout(encoded_layers3)
        
        '''
        score = torch.tanh(torch.matmul(sequence_output, self.W_w))
        attention_weights = F.softmax(torch.matmul(score, self.u_w), dim=1)
        sequence_output = sequence_output* attention_weights 
        
        encoded_layers = sequence_output.permute(0, 2, 1)
        ...
        print(sequence_output.shape,cat.shape)
        '''
...
def evaluate(config, model, tokenizer, prefix=""):
    ...
    dic_triple={str(i):set() for i in range(1500)}
    
    output_eval_file = "sem_res.txt"
    # print(lines)
    with open(output_eval_file, "w") as writer:
        for key in range(len(preds)):
            ...
            # l=[]
            # l.append()
            triple=str(tuple(([e1,str(RELATION_LABELS[preds[key]]),e2])))
            
            dic_triple[lines[key][3]].add(triple)
            writer.write("%d\t%s\t%s\t%s\t%s\n" %(key,lines[key][0], lines[key][1],str(RELATION_LABELS[preds[key]]),lines[key][3]))
...
def main():
    ...
    parser.add_argument('--config',default='pipe_config.ini', dest='config')
    ...
    #      "/home/sda/chenyanguang/RE/bert-pipeline/Robera", num_labels=num_labels, finetuning_task=config.task_name)
    # bertconfig.l2_reg_lambda = config.l2_reg_lambda
    # tokenizer = BertTokenizer.from_pretrained(
    #      "/home/sda/chenyanguang/RE/bert-pipeline/Robera", do_lower_case=True, additional_special_tokens=additional_special_tokens)
    # model = BertForSequenceClassification.from_pretrained(
    #      "/home/sda/chenyanguang/RE/bert-pipeline/Robera", config=bertconfig)
    bertconfig = BertConfig.from_pretrained(
         'bert-base-chinese', num_labels=num_labels, finetuning_task=config.task_name)
    bertconfig.l2_reg_lambda = config.l2_reg_lambda
    tokenizer = BertTokenizer.from_pretrained(
         'bert-base-chinese', do_lower_case=True, additional_special_tokens=additional_special_tokens)
    model = BertForSequenceClassification.from_pretrained(
