...
def save_data_setting(data, save_file):
    ...
    new_data.raw_Ids = []
    ## save data settings
    with open(save_file, 'wb') as fp:#,open(configure.trsaveset, 'wb') as fp1:
...
def load_data(status='train'):   #处理数据
    public_path = configure.public_path

    # Path to load dataset (folder name)
    dataset = os.path.join(public_path,configure.dataset)  #数据集位置
    ...
    weights_mode = configure.weights_mode.lower() #weights_mode = 'smooth'
    gpu = torch.cuda.is_available()    #gpu==cuda

    # Character Embeddings
    char_emb = os.path.join(public_path,configure.char_emb_file)
    #char_emb = os.path.join(configure.char_emb_file)
    # Bi-gram  Embeddings
    bichar_emb = None  #是否是该字的前后二字词
    # Word sense Embeddings
    gaz_file = os.path.join(public_path,configure.sense_emb_file)  #sense.txt
