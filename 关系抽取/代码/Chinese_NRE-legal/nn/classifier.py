...
class AttClassifier(nn.Module):
    def __init__(self, data):
        ...
        self.rnn_ne = nn.GRU(self.gru_size*2, self.gru_size, 
                             batch_first=True)
    ...
    def get_logit(self, hidden_out, ins_label, scope):
        ...
        if self.training:
            ...
            for i in range(self.batch_size):
                #print(i,self.batch_size,scope[i][0],scope[i][1],attention_r.shape)
                ...
                # Bag representations
                bag_repre_for_each_rel =torch.matmul(attention_score,bag_hidden_mat) #shape: 1,200
                #print(bag_repre_for_each_rel .shape,bag_repre_for_each_rel.view(self.gru_size).shape)  
                bag_repre.append(bag_repre_for_each_rel.view(self.gru_size)) # [1 * gru_size]
            bag_repre = torch.stack(bag_repre)  #stack会新加增加一个维度，可理解为叠加
            # [batch_size * gru_size] /dot [gru_size * num_classes] -> [batch_size * num_classes]
            ...
            for i in range(self.batch_size):                
                ...
                bag_repre_for_each_rel = torch.matmul(attention_score,bag_hidden_mat)  #shape: 5,200
                #print(bag_repre_for_each_rel .shape)
